...
local cp = require("catppuccin-mocha")
...
hl.on("hyprland.start", function()
	...
	-- nm-applet disabled — network info is in the DMS bar
...
-- If cursor becomes invisible, uncomment:
...
hl.config({
	general = {
		...
		col = {
			active_border = { colors = { cp.mauve, cp.pink }, angle = 45 },
			inactive_border = cp.surface0,
		},
...
hl.config({
	input = {
		...
		repeat_delay = 200, -- ms before repeating starts (default: 600)
...
hl.bind(mainMod .. " + A", hl.dsp.exec_cmd("~/.config/hypr/scripts/reload-dms.sh"))
...
-- Switch workspaces: 1-4 with mainMod + CTRL + [1-4], 5-10 with mainMod + [5-0]
for i = 1, 4 do
...
-- Launch/focus apps with mainMod + [1-4]
-- hl.bind(mainMod .. " + 1", hl.dsp.exec_cmd("~/.config/hypr/scripts/focus-or-launch.sh brave-browser brave"))
-- hl.bind(mainMod .. " + 2", hl.dsp.exec_cmd("~/.config/hypr/scripts/focus-or-launch.sh com.mitchellh.ghostty ghostty"))
-- hl.bind(mainMod .. " + 3", hl.dsp.exec_cmd("~/.config/hypr/scripts/focus-or-launch.sh dolphin dolphin"))
-- hl.bind(mainMod .. " + 4", hl.dsp.exec_cmd("~/.config/hypr/scripts/focus-or-launch.sh spotify spotify"))
--
...
-- Multimedia keys: volume/mic/media via DMS IPC (shows DMS popups);
-- external monitor brightness stays on DDC/CI
hl.bind(
...
hl.bind(
	"XF86MonBrightnessUp",
	hl.dsp.exec_cmd("~/.config/hypr/scripts/monitor-brightness.sh up"),
	{ locked = true, repeating = true }
)
hl.bind(
	"XF86MonBrightnessDown",
	hl.dsp.exec_cmd("~/.config/hypr/scripts/monitor-brightness.sh down"),
	{ locked = true, repeating = true }
)
...
-- DMS: no animations on shell layers (bar, spotlight, lock, popups)
hl.layer_rule({
	match = { namespace = "dms" },
	no_anim = true,
})

-- DMS integration: files generated by DMS (Settings → Compositor).
-- Placeholders exist in ./dms/ so the requires succeed before first run.
require("dms.colors")
require("dms.layout")
require("dms.outputs")
